        ...
        for x in range(no_of_var):
            ...
            for y in Neigh_x:
                ...
                for S in combinations(Neigh_x_noy, depth):
                    p = cg.ci_test(x, y, S)
                    #"Set the threshold for using LLM."
                    threshold = 0.001#raw= 0.001
                    #ADD LLM
                    if p > alpha and (p - alpha) > threshold:
                        if verbose:
                    ...
                    elif abs(p - alpha) < threshold:
                        for_llm = [FACTOR_list[x],FACTOR_list[y],[FACTOR_list[i] for i in S]]
                        Prompt = propt_test(for_llm,var_names)
                        score = individual_test(Prompt)
                        if  score  >= 0.5:
                            if verbose:
                                print('%d ind %d | %s with p-value %f\n' % (x, y, S, p))
                            if not stable:
